...
def process_data(df, video_type, threshold):
    ...
    for channel in channels:
        ...
        st.write(f'<b>{channel}</b>의 가장 큰 기울기: <span style="color:blue;">{max_slope:.2f}</span> (기간: {max_year_week})', unsafe_allow_html=True)
        
        if max_slope > threshold:
            st.write(f'<span style="color:red; font-weight:bold;">{channel}는 떡상 기준에 적합합니다!</span> (기울기: {max_slope:.2f} > {threshold})', unsafe_allow_html=True)
        else:
            st.write(f'<span style="color:gray;">{channel}는 떡상 기준에 부적합합니다!</span> (기울기: {max_slope:.2f} < {threshold})', unsafe_allow_html=True)
